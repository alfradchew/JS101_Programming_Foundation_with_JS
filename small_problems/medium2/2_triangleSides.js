...
side, and every side must have a length greater than 0. If either of these conditions is not satisfied, the triangle is
invalid.
...
let triangle = (num1, num2, num3) => {
  sortedLength = [num1, num2, num3].sort((a, b) => a - b);
  if ((sortedLength.includes(0) || (sortedLength[0] + sortedLength[1]) <= sortedLength[2])) {
    return 'invalid';
  } else if (num1 === num2 && num2 === num3) {
    return 'equilateral';
  } else if (sortedLength[1] === sortedLength[0] || sortedLength[1] === sortedLength[2]) {
    return 'isosceles';
  } else {
    return 'scalene';
  }
