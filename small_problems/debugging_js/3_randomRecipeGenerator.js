/*
One bored and hungry evening we decided to randomly generate recipes. We can't wait to see the first suggestions, but JavaScript raises a TypeError,
...
// dishname and dish are string, which does not have the join method. Only arrays have the join method.
// change dishName and dish to:
// let dishName = random(adjective).concat(random(firstNoun)).concat(random(secondNoun))
// let dish = random(ingredients, 3).concat(random(spices, 2)).concat(random(extras, 1));