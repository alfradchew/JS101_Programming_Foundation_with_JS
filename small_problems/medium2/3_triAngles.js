...
To be a valid triangle, the sum of the angles must be exactly 180 degrees, and every angle must be greater than 0. If
...
let triangle = (num1, num2, num3) => {
  sortedAngel = [num1, num2, num3].sort((a, b) => a - b);
  if ((sortedAngel.includes(0) || sortedAngel.reduce((total, angle) => total + angle) !== 180)) {
    return 'invalid';
  } else if (sortedAngel.includes(90)) {
    return 'right';
  } else if (sortedAngel.every(angle => angle < 90)) {
    return 'acute';
  } else {
    return 'obtuse';
  }
