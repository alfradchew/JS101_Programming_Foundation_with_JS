...
Write two functions: one that takes a Rational number as an argument, and returns an Array of the denominators that are
part of an Egyptian Fraction representation of the number, and another that takes an Array of numbers in the same format,
...
let egyptian = frac => {
  ...
  while (frac.n !== 0) {
    let currentFraction = new Fraction(1, currentDenominator);
    if (currentFraction <= frac) {
      denominatorArray.push(currentDenominator);
      frac = frac.sub(currentFraction);
    }
...
let unegyptian = arr => {
  return arr.reduce((accumulator, denominator) => accumulator + new Fraction(1, denominator), new Fraction(0));
};
...
/*
Every rational number can be expressed as an Egyptian Fraction. In fact, every rational number can be expressed as an
Egyptian Fraction in an infinite number of different ways. Thus, the first group of examples may not show the same values
as your solution. They do, however, show the expected form of the solution. The remaining examples merely demonstrate that
the output of egyptian can be reversed by unegyptian.
*/