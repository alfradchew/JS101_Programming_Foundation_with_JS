/*
We want to implement a role-playing game and started working on the dice roll functionality. First, study the game code. Then take a look at the example
...
/*
When playing around with the above program, our two test rolls result in two random values that produce the sample output below (because each dice roll
produces a random value, your output may differ). The outcome of rolling 16 looks correct, but the output when we rolled values 1 and 20 doesn't make
sense. For each roll, only one outcome should be displayed. What is wrong with the code?
...
// Because there is a lack of break statement in the switch statements.

/*
Further Exploration

This behavior of switch statements might seem strange at first, but there are scenarios where it's handy to "fall through" to subsequent cases, for example
when you want the same statement to be executed in several cases. Here is one such example, where our educated role-playing character comments on the
